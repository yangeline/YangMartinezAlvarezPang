...
```{r}
State_GDP_ranked <- States_GDP[order(States_GDP$GDP2021, decreasing = TRUE), ]

# Calculate GDP growth rates for 2009 to 2021
States_GDP$GrowthRate <- (States_GDP$GDP2021 / States_GDP$GDP2009) - 1
...
emissions_comparison_plot <- ggplot(similar_states_emissions, aes(x = State, y = `Emission 2021`, fill = State)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  ggtitle("CO2 Emissions in 2021 for States with GDP Growth Similar to North Carolina") +
  xlab("State") +
  ylab("CO2 Emissions in 2021") +
